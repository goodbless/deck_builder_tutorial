...
func add_gold_reward(amount: int) -> void:
	...
	gold_reward.reward_text = GOLD_TEXT % str(amount)
...
func _show_card_rewards() -> void:
	...
	for i in run_stats.card_rewards:
		...
		for rarity: Card.Rarity in card_reward_weights:
			if card_reward_weights[rarity] > roll:
...
func _setup_card_chances() -> void:
	...
	card_reward_weights[Card.Rarity.COMMON] = run_stats.common_weight
	card_reward_weights[Card.Rarity.UNCOMMON] = run_stats.common_weight + run_stats.uncommon_weight
	card_reward_weights[Card.Rarity.RARE] = card_reward_total_weight
...
func _get_random_available_card(available_cards: Array[Card], with_rarity: Card.Rarity) -> Card:
	var all_available_cards := available_cards.filter(
		func(card: Card) -> bool:
			return card.rarity == with_rarity
	)
	return all_available_cards.pick_random()
